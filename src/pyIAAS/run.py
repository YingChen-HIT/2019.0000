...
def record_pool_statistic(cfg, env_) -> None:
    """
    record pool statistic to file
    """
    import json
    save_path = os.path.join(cfg.NASConfig['OUT_DIR'], 'pool_statistic.json')
    ...
    # append new data
    data.append({
        'pool_size': len(env_.net_pool),
        'id': [i.index for i in env_.net_pool], 
        'performance': [i.test_loss_best for i in env_.net_pool],
    })
