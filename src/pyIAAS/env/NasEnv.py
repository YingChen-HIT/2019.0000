...
class NasEnv(gym.Env):
    ...
    def step(self, action: Any):
        ...
        if "RandomDrop" not in self.cfg.NASConfig or not self.cfg.NASConfig['RandomDrop']:
            self.logger.info(f"drop {len(self.net_pool) - self.pool_size} network by heuristic")
            self.net_pool = sorted(self.net_pool, key=lambda x: x.test_loss_best)
        else:
            assert self.cfg.NASConfig['RandomDrop']
            self.logger.info(f"drop {len(self.net_pool) - self.pool_size} network randomly")
            random.shuffle(self.net_pool)
    ...
    def get_reward(self):
        ...
        reward_dict = {}
        for net in self.net_pool:
            regularize_layer_number = self.cfg.NASConfig['RewardRegularisationLayers'] * max(0,
                                                                                             len(net.model_config.modules) -
                                                                                             self.cfg.NASConfig[
                                                                                                 'LayerNumberStartRegularize'])
            regularize_layer_width = self.cfg.NASConfig['RewardRegularisationWidth'] * max(0, max(
                i.current_level for i in net.model_config.modules) -
                                                                                           self.cfg.NASConfig[
                                                                                               'LayerWidthStartRegularize'])
    ...
    def reset(self):
        ...
        X_train, y_train, X_test, y_test = self.train_test_data
        self.net_pool = [
            generate_new_model_config(self.cfg, self.feature_shape, self.target_shape, [i]).generate_model() for i in
            self.cfg.modulesConfig.keys()]
        if len(self.net_pool) < self.pool_size:
            self.net_pool.extend(self.generate_random_net(self.pool_size - len(self.net_pool)))
        self._train_and_test()
        self.render()
    ...
    def _train_and_test(self):
        ...
        for net in self.net_pool:
            if self.cfg.NASConfig['GPU']:  # use gpu
                net.to_cuda()
            net.train(X_train, y_train)
            net.test(X_test, y_test)
            if self.cfg.NASConfig['GPU']:  # release gpu memory
                net.to_cpu()
