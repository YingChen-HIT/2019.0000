    ...
    def init_param(self, input_shape, max_width):
        ...
        if self.cfg.NASConfig['Pruning']:
            ...
            def weight_hh_hook(grad):
                ...
                grad_calculated = torch.sum(grad_calculated, 0)
                accumulate_importance(grad_calculated)
                return grad
...
class LSTMCell(nn.Module):
    ...
    def forward(self, input: Tensor, h, c):
        ...
        outgate = torch.clip(outgate, 0, 1)  # clip to make sure output range is the same as original LSTM

        cy = (forgetgate * cx) + (ingate * cellgate)
        hy = outgate * torch.tanh(cy) + outgate
        hy = torch.clip(hy, -1, 1) # clip to mak e sure output range is the same as original LSTM
